                         ...
                         ignition::gazebo::EventManager &_eventMgr)
{
    ...
    {
        ignerr << "MyJointController plugin should be attached to a model entity. "
    ...
    // Get params from SDF
    sdf::ElementPtr sdfElem = ptr->GetElement("joint");
    while (sdfElem)
    {
        this->dataPtr->JointNames.push_back(sdfElem->Get<std::string>());
        sdfElem = sdfElem->GetNextElement("joint");
    }
    ...
    std::cout << "\t Answer:\t" << sdfElem << std::endl;
    ...
    //std::cout << "\t Link Entity Name:\t" << std::endl;
}
...
{
    //std::cout << "\t Link Entity Name:\t" << std::endl;
    IGN_PROFILE("MyJointControllerPlugin::PreUpdate");
    ...
    for (ignition::gazebo::Entity joint : this->dataPtr->Joints)
    {
        ...
        _ecm.CreateComponent(
            joint, ignition::gazebo::components::JointVelocityCmd({this->dataPtr->JointSpeed}));
        }
        else
        {
        *vel = ignition::gazebo::components::JointVelocityCmd({this->dataPtr->JointSpeed});
        }
    }
    ...
    auto leftPos = _ecm.Component<ignition::gazebo::components::JointPosition>(
        this->dataPtr->Joints[0]);
    if (!leftPos)
    {
        _ecm.CreateComponent(this->dataPtr->Joints[0],
...
{
    IGN_PROFILE("MyJointControllerPlugin::PostUpdate");
    // Nothing left to do if paused.
    if (_info.paused)
        return;
...
void MyJointControllerPluginPrivate::UpdateOdometry(const ignition::gazebo::UpdateInfo &_info,
                                                    const ignition::gazebo::EntityComponentManager &_ecm)
{
    IGN_PROFILE("MyJointControllerPlugin::UpdateOdometry");
    ...
    auto leftPos = _ecm.Component<ignition::gazebo::components::JointPosition>(this->Joints[0]);

    // Abort if the joints were not found or just created.
    if (!leftPos || leftPos->Data().empty())
    {
        return;
    }
    /*this->odom.Update(leftPos->Data()[0],
    ...
    std::cout << "\t Link Entity Name:\t" << JointSpeed << std::endl;
...
void MyJointControllerPluginPrivate::UpdateVelocity(const ignition::gazebo::UpdateInfo &_info,
                                                    const ignition::gazebo::EntityComponentManager &_ecm)
{
    IGN_PROFILE("MyJointControllerPlugin::UpdateVelocity");
...
void MyJointControllerPluginPrivate::UpdatePose(const ignition::gazebo::UpdateInfo &_info,
                                                const ignition::gazebo::EntityComponentManager &/*_ecm*/)
{
  IGN_PROFILE("MyJointControllerPlugin::UpdatePose");

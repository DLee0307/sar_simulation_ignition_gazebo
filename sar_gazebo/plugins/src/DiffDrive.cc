...
class ignition::gazebo::systems::DiffDrivePrivate
{
  ...
  /// \brief Callback for pose subscription
  /// \param[in] _msg Pose message
  public: void OnCmdPose(const ignition::msgs::Pose &_msg);
  ...
  /// \param[in] _ecm The EntityComponentManager of the given simulation
  /// instance.
  public: void UpdatePose(const ignition::gazebo::UpdateInfo &_info,
    const ignition::gazebo::EntityComponentManager &_ecm);
  ...
  /// \brief Entity of the model
  public: std::vector<Entity> models;
  ...
  /// \brief Last target pose requested.
  public: msgs::Pose targetPose;
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  this->dataPtr->node.Subscribe("/pose_publisher", &DiffDrivePrivate::OnCmdPose, this->dataPtr.get());
    ...
    ignition::gazebo::EntityComponentManager &_ecm)
{
  ...
  auto pose = convert<math::Pose3d>(this->dataPtr->targetPose);
  this->dataPtr->model.SetWorldPoseCmd(_ecm, pose);

  // auto worldPose =
  //     _ecm.Component<components::WorldPose>(this->dataPtr->models[0]);
  // auto modelPose = _ecm.Component<components::WorldPoseCmd>(
  //     this->dataPtr->models[0]);
  // if (!modelPose)
  // {
  //   _ecm.CreateComponent(this->dataPtr->models[0],
  //       components::WorldPoseCmd({this->dataPtr->modelPose}));
  //   std::cout << "c-001" << std::endl;
  // }
  // else
  // {
  //   *modelPose = components::WorldPoseCmd({this->dataPtr->modelPose});
  //   std::cout << "c-002" << std::endl;
  // }
}
...
//////////////////////////////////////////////////
void DiffDrivePrivate::UpdatePose(const ignition::gazebo::UpdateInfo &_info,
    const ignition::gazebo::EntityComponentManager &/*_ecm*/)
{
  IGN_PROFILE("DiffDrive::UpdatePose");

  auto pose = convert<math::Pose3d>(this->targetPose);
  // math::Pose3d pose;
...
void DiffDrivePrivate::OnCmdPose(const msgs::Pose &_msg)
{
  std::lock_guard<std::mutex> lock(this->mutex);
